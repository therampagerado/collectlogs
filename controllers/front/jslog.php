...
class CollectLogsJsLogModuleFrontController extends ModuleFrontController
{
    ...
    public function postProcess()
    {
        ...
        try {
            ...
            PrestaShopLogger::addLog('collectlogs jslog failed: ' . $e->getMessage(), 3);
    ...
    protected function allowRequestRate()
    {
        ...
        return $this->allowRequestRateFallback($ipHash, $visitor);
    }

    protected function hasRateLimitTable()
    {
        ...
        try {
            $table = _DB_PREFIX_ . 'collectlogs_js_rate_limit';
            $result = Db::getInstance()->getValue("SHOW TABLES LIKE '" . pSQL($table) . "'");
            $hasTable = (bool)$result;
        } catch (Throwable $e) {
            $hasTable = false;
        }
        return $hasTable;
    ...
    protected function allowRequestRateFallback($ipHash, $visitor)
    {
        $db = Db::getInstance();
        $where = "last_seen >= DATE_SUB(NOW(), INTERVAL 1 MINUTE) AND ip_hash = '" . pSQL($ipHash) . "'";
        if ($visitor) {
            $where .= " AND visitor_id = '" . pSQL($visitor) . "'";
        }
